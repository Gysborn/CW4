...
def compare_passwords(password_hash, password) -> bool:
...
def generate_tokens(email, password, password_hash=None, is_refresh=False):
    if email is None:
        return None
    if not is_refresh:
        if not compare_passwords(password=password, password_hash=password_hash):
            return None
...
def get_decode_token(refresh_token):
    try:
        data = jwt.decode(jwt=refresh_token, key=current_app.config['SECRET_KEY'],
                          algorithms=[current_app.config['ALGORITHM']])
        return data
